...
public class UpdatePhone extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        DaoUtil.getPhoneDao().update(phoneId, new PhoneSpecification(new Date(), null, minsCount, smsCount));
        response.sendRedirect(request.getContextPath() + "/showPhone");
    }

    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String phoneId = request.getParameter("phoneId");
        List<PhoneSpecification> history = DaoUtil.getPhoneDao().getHistory(Integer.parseInt(phoneId));
