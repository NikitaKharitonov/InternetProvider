...
public class UpdatePhone extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        DaoUtil.getPhoneDao().update(phoneId, new TemporalPhone(new Date(), null, minsCount, smsCount));
        response.sendRedirect(request.getContextPath() + "/showPhone");
    }

    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String phoneId = request.getParameter("phoneId");
        List<TemporalPhone> history = DaoUtil.getPhoneDao().getHistory(Integer.parseInt(phoneId));
